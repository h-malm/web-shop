...
export default function BrowseCards () {
    ...
    const addItem = useCallback( async ( item ) => {
        if ( currentUser && item.owner === currentUser ) {
            toast.current.show( {
                severity: 'warn',
                detail: 'You cannot add your own items to the cart.'
            } );
            return;
        }
        ...
        if ( userLoggedOut ) {
            toast.current.show( {
                severity: 'warn',
                detail: 'You need to sign in to add items to the cart'
            } );
            return;
        }
        ...
        try {
            ...
            toast.current.show( {
                severity: 'success',
                detail: `Added '${item.title} ' to cart`
            } )
            await refreshCart();
        } catch ( error ) {
    ...
    const init = useCallback( async ( search ) => {
        try {
            const url = search ?
    ...
    const removeItem = async ( id ) => {
        ...
        try {
            ...
            toast.current.show( {
                severity: 'info',
                detail: `Removed '${cardToRemove?.title}' item from cart`
            } )
            await refreshCart();
        } catch ( error ) {
